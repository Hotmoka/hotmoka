...
public abstract class Update extends Marshallable implements Comparable<Update> {
	...
	public static Update from(UnmarshallingContext context) throws IOException, ClassNotFoundException {
		...
		case UpdateOfBigInteger.SELECTOR_BALANCE: return new UpdateOfBigInteger(StorageReference.from(context), FieldSignature.BALANCE_FIELD, unmarshallBigInteger(context));
		case UpdateOfBigInteger.SELECTOR_GAS_PRICE: return new UpdateOfBigInteger(StorageReference.from(context), FieldSignature.GENERIC_GAS_STATION_GAS_PRICE_FIELD, unmarshallBigInteger(context));
		case UpdateOfBigInteger.SELECTOR_UBI_VALUE: return new UpdateOfBigInteger(StorageReference.from(context), FieldSignature.UNSIGNED_BIG_INTEGER_VALUE_FIELD, unmarshallBigInteger(context));
		case UpdateOfBigInteger.SELECTOR_RED_BALANCE: return new UpdateOfBigInteger(StorageReference.from(context), FieldSignature.RED_BALANCE_FIELD, unmarshallBigInteger(context));
		...
		case UpdateOfBigInteger.SELECTOR_NONCE: return new UpdateOfBigInteger(StorageReference.from(context), FieldSignature.EOA_NONCE_FIELD, unmarshallBigInteger(context));
		case UpdateOfBigInteger.SELECTOR: return new UpdateOfBigInteger(StorageReference.from(context), FieldSignature.from(context), unmarshallBigInteger(context));
		...
		case UpdateOfByte.SELECTOR: return new UpdateOfByte(StorageReference.from(context), FieldSignature.from(context), context.ois.readByte());
		case UpdateOfChar.SELECTOR: return new UpdateOfChar(StorageReference.from(context), FieldSignature.from(context), context.ois.readChar());
		case UpdateOfDouble.SELECTOR: return new UpdateOfDouble(StorageReference.from(context), FieldSignature.from(context), context.ois.readDouble());
		case UpdateOfEnumEager.SELECTOR: return new UpdateOfEnumEager(StorageReference.from(context), FieldSignature.from(context), context.ois.readUTF(), context.ois.readUTF());
		case UpdateOfEnumLazy.SELECTOR: return new UpdateOfEnumLazy(StorageReference.from(context), FieldSignature.from(context), context.ois.readUTF(), context.ois.readUTF());		
		case UpdateOfFloat.SELECTOR: return new UpdateOfFloat(StorageReference.from(context), FieldSignature.from(context), context.ois.readFloat());
		case UpdateOfInt.SELECTOR: return new UpdateOfInt(StorageReference.from(context), FieldSignature.from(context), context.ois.readInt());
		case UpdateOfInt.SELECTOR_SMALL: return new UpdateOfInt(StorageReference.from(context), FieldSignature.from(context), context.ois.readShort());
		case UpdateOfInt.SELECTOR_VERY_SMALL: return new UpdateOfInt(StorageReference.from(context), FieldSignature.from(context), context.ois.readByte());
		case UpdateOfInt.SELECTOR_STORAGE_TREE_MAP_NODE_SIZE: return new UpdateOfInt(StorageReference.from(context), FieldSignature.STORAGE_TREE_MAP_NODE_SIZE_FIELD, context.ois.readInt());
		case UpdateOfInt.SELECTOR_STORAGE_TREE_INTMAP_NODE_SIZE: return new UpdateOfInt(StorageReference.from(context), FieldSignature.STORAGE_TREE_INTMAP_NODE_SIZE_FIELD, context.ois.readInt());
		case UpdateOfInt.SELECTOR_STORAGE_TREE_INTMAP_NODE_KEY: return new UpdateOfInt(StorageReference.from(context), FieldSignature.STORAGE_TREE_INTMAP_NODE_KEY_FIELD, context.ois.readInt());
		case UpdateOfLong.SELECTOR: return new UpdateOfLong(StorageReference.from(context), FieldSignature.from(context), context.ois.readLong());
		case UpdateOfShort.SELECTOR: return new UpdateOfShort(StorageReference.from(context), FieldSignature.from(context), context.ois.readShort());
		...
		case UpdateOfString.SELECTOR_PUBLIC_KEY: return new UpdateOfString(StorageReference.from(context), FieldSignature.EOA_PUBLIC_KEY_FIELD, context.ois.readUTF());
		case UpdateOfString.SELECTOR: return new UpdateOfString(StorageReference.from(context), FieldSignature.from(context), context.ois.readUTF());
