...
public abstract class ExecutionEnvironment {
	...
	protected final ConsensusConfig<?,?> extractConsensus(StorageReference manifest) throws InterruptedException {
		try {
			...
			StorageReference validators = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_VALIDATORS + " should not return void"))
					.asReturnedReference(MethodSignatures.GET_VALIDATORS, LocalNodeException::new);
			...
			StorageReference gasStation = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_GAS_STATION + " should not return void"))
					.asReturnedReference(MethodSignatures.GET_GAS_STATION, LocalNodeException::new);
			...
			StorageReference versions = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_VERSIONS + " should not return void"))
					.asReturnedReference(MethodSignatures.GET_VERSIONS, LocalNodeException::new);
			...
			String genesisTime = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_GENESIS_TIME + " should not return void"))
					.asReturnedString(MethodSignatures.GET_GENESIS_TIME, LocalNodeException::new);
			...
			String chainId = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_CHAIN_ID + " should not return void"))
					.asReturnedString(MethodSignatures.GET_CHAIN_ID, LocalNodeException::new);
	
			StorageReference gamete = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					(manifest, _100_000, takamakaCode, MethodSignatures.GET_GAMETE, manifest))
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_GAMETE + " should not return void"))
					.asReturnedReference(MethodSignatures.GET_GAMETE, LocalNodeException::new);
			...
			int maxDependencies = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.asReturnedInt(MethodSignatures.GET_MAX_DEPENDENCIES, LocalNodeException::new);
			...
			boolean allowsFaucet = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.asReturnedBoolean(MethodSignatures.ALLOWS_UNSIGNED_FAUCET, LocalNodeException::new);
			...
			boolean skipsVerification = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.SKIPS_VERIFICATION + " should not return void"))
					.asReturnedBoolean(MethodSignatures.SKIPS_VERIFICATION, LocalNodeException::new);
			...
			String signature = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_SIGNATURE + " should not return void"))
					.asReturnedString(MethodSignatures.GET_SIGNATURE, LocalNodeException::new);
			...
			BigInteger ticketForNewPoll = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_TICKET_FOR_NEW_POLL + " should not return void"))
					.asReturnedBigInteger(MethodSignatures.GET_TICKET_FOR_NEW_POLL, LocalNodeException::new);
			...
			BigInteger initialGasPrice = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_INITIAL_GAS_PRICE + " should not return void"))
					.asReturnedBigInteger(MethodSignatures.GET_INITIAL_GAS_PRICE, LocalNodeException::new);
			...
			BigInteger maxGasPerTransaction = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_MAX_GAS_PER_TRANSACTION + " should not return void"))
					.asReturnedBigInteger(MethodSignatures.GET_MAX_GAS_PER_TRANSACTION, LocalNodeException::new);
			...
			boolean ignoresGasPrice = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.IGNORES_GAS_PRICE + " should not return void"))
					.asReturnedBoolean(MethodSignatures.IGNORES_GAS_PRICE, LocalNodeException::new);
			...
			BigInteger targetGasAtReward = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_TARGET_GAS_AT_REWARD + " should not return void"))
					.asReturnedBigInteger(MethodSignatures.GET_TARGET_GAS_AT_REWARD, LocalNodeException::new);
			...
			long oblivion = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.orElseThrow(() -> new LocalNodeException(MethodSignatures.GET_OBLIVION + " should not return void"))
					.asReturnedLong(MethodSignatures.GET_OBLIVION, LocalNodeException::new);
			...
			BigInteger initialSupply = runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall
					...
					.asReturnedBigInteger(MethodSignatures.GET_INITIAL_SUPPLY, LocalNodeException::new);
	...
	protected final StorageReference extractValidators(StorageReference manifest) {
		try {
			...
			throw new LocalNodeException("The manifest does not contain the reference to the validators set", e);
		}
		catch (UnknownReferenceException e) {
			throw new LocalNodeException("The manifest is set but cannot be found in store", e);
		}
	}
	...
	protected final StorageReference extractGasStation(StorageReference manifest) {
		try {
			...
			throw new LocalNodeException("The manifest does not contain the reference to the gas station", e);
		}
		catch (UnknownReferenceException e) {
			throw new LocalNodeException("The manifest is set but cannot be found in store", e);
		}
	...
	protected final StorageReference extractVersions(StorageReference manifest) {
		try {
			...
			throw new LocalNodeException("The manifest does not contain the reference to the versions manager", e);
		}
		catch (UnknownReferenceException e) {
			throw new LocalNodeException("The manifest is set but cannot be found in store", e);
		}
	...
	protected final BigInteger extractGasPrice(StorageReference manifest) throws InterruptedException {
		...
		try {
			return runInstanceMethodCallTransaction(TransactionRequests.instanceViewMethodCall(manifest, _100_000, takamakaCode, GET_GAS_PRICE, gasStation))
					.orElseThrow(() -> new LocalNodeException(GET_GAS_PRICE + " should not return void"))
					.asReturnedBigInteger(GET_GAS_PRICE, LocalNodeException::new);
		}
	...
	protected final UpdateOfField getLastUpdateToField(StorageReference object, FieldSignature field) throws UnknownReferenceException, FieldNotFoundException {
		...
		try {
			...
			throw new LocalNodeException("Object " + object + " has a history containing a reference not in store");
		}
	}
	...
	protected final Optional<StorageReference> getGamete() {
		...
		try {
			...
			throw new LocalNodeException("The manifest does not contain the reference to the gamete", e);
		}
		catch (UnknownReferenceException e) {
	...
	protected final Stream<Update> getUpdates(TransactionReference referenceInHistory) {
		try {
			if (getResponse(referenceInHistory) instanceof TransactionResponseWithUpdates trwu)
				...
				throw new LocalNodeException("Transaction " + referenceInHistory + " belongs to the histories but does not contain updates");
		}
		catch (UnknownReferenceException e) {
			throw new LocalNodeException("Transaction " + referenceInHistory + " belongs to the histories but is not present in store");
		}
	}
	...
	protected final ResponseBuilder<?,?> responseBuilderFor(TransactionReference reference, TransactionRequest<?> request) {
		if (request instanceof JarStoreInitialTransactionRequest jsitr)
			...
			throw new LocalNodeException("Unexpected transaction request of class " + request.getClass().getName());
	}
