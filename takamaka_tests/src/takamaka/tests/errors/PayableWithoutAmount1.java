...
import io.takamaka.instrumentation.issues.PayableWithoutAmountError;
import takamaka.blockchain.request.JarStoreTransactionRequest;
import takamaka.memory.InitializedMemoryBlockchain;
import takamaka.tests.TakamakaTest;

class PayableWithoutAmount1 extends TakamakaTest {
	...
	void installJar() {
		throwsVerificationExceptionWithCause(PayableWithoutAmountError.class, () ->
			blockchain.addJarStoreTransaction
				(new JarStoreTransactionRequest(blockchain.account(0), _20_000, blockchain.takamakaBase,
				Files.readAllBytes(Paths.get("../takamaka_examples/dist/payablewithoutamount1.jar")), blockchain.takamakaBase))
		);
	}
}