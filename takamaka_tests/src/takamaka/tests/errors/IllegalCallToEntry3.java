...
import io.takamaka.code.instrumentation.issues.IllegalCallToEntryError;
import takamaka.blockchain.request.JarStoreTransactionRequest;
import takamaka.memory.InitializedMemoryBlockchain;
import takamaka.tests.TakamakaTest;

class IllegalCallToEntry3 extends TakamakaTest {
	...
	void installJar() {
		throwsVerificationExceptionWithCause(IllegalCallToEntryError.class, () ->
			blockchain.addJarStoreTransaction
				(new JarStoreTransactionRequest(blockchain.account(0), _20_000, blockchain.takamakaBase,
				Files.readAllBytes(Paths.get("../takamaka_examples/dist/illegalcalltoentry3.jar")), blockchain.takamakaBase))
		);
	}
}