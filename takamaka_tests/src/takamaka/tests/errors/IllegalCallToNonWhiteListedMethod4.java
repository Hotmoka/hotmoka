...
class IllegalCallToNonWhiteListedMethod4 {
	...
	void installJar() throws TransactionException, CodeExecutionException, IOException {
		TransactionReference jar = blockchain.addJarStoreTransaction
			...
			Files.readAllBytes(Paths.get("../takamaka_examples/dist/illegalcalltononwhitelistedmethod4.jar")), blockchain.takamakaBase));		
		...
		try {
			blockchain.addStaticMethodCallTransaction(new StaticMethodCallTransactionRequest
				...
				new NonVoidMethodSignature(new ClassType("takamaka.tests.errors.illegalcalltononwhitelistedmethod4.C"), "foo", ClassType.STRING)));
		}
