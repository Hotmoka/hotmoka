...
class ThrowsExceptionsOnNonPublic1 {
	...
	void installJar() throws TransactionException, CodeExecutionException, IOException {
		try {
			blockchain.addJarStoreTransaction
				...
				Files.readAllBytes(Paths.get("../takamaka_examples/dist/throwsexceptionsonnonpublic1.jar")), blockchain.takamakaBase));		
		}
		catch (TransactionException e) {
			if (e.getCause() instanceof VerificationException
					&& ((VerificationException) e.getCause()).getError() instanceof takamaka.verifier.ThrowsExceptionsOnNonPublicError)
				return;
