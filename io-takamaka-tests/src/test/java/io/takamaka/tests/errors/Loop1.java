...
class Loop1 extends TakamakaTest {
	...
	void installJar() throws TransactionException, IOException {
		addJarStoreTransaction(account(0), _20_000, BigInteger.ONE, takamakaCode(), bytesOf("loop1.jar"), takamakaCode());
	}
	...
	void callLoop() throws TransactionException, IOException, CodeExecutionException {
		TransactionReference loop = addJarStoreTransaction(account(0), _20_000, BigInteger.ONE, takamakaCode(), bytesOf("loop1.jar"), takamakaCode());

		TakamakaTest.throwsTransactionExceptionWithCause(NonWhiteListedCallException.class, () -> 
			addStaticMethodCallTransaction(account(0), _20_000, BigInteger.ONE, new Classpath(loop, true), new VoidMethodSignature("io.takamaka.tests.errors.loop1.Loop", "loop")));
	}
}