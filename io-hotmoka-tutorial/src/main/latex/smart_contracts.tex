...
The use of \texttt{BigIntegerSupport}\index{BigIntegerSupport@{\texttt{BigIntegerSupport}}}
...
its state. For more information, see \emph{unexpected Ether} in~\cite{AntonopoulosW19}.
\end{commentbox}
...
\item The return value of \texttt{caller()} must be cast to \texttt{PayableContract}, which is
  safe because of point~\ref{enum:invest_onlly_payable} above.
\end{enumerate}
%
...
\begin{figure}[th]
  \begin{center}
    \includegraphics[width=0.61\textwidth]{pics/contracts}
  \end{center}
