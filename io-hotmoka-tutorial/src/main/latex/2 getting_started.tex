...
a blockchain. Note that Hotmoka itself is not a bockchain: it is the
definition of an abstract machine, of its states and of the transactions
that modify that state. Hotmoka becomes a blockchain only if it runs on top
of a software component that takes care of networking and consensus among
the many peers of a blockchain. Currently, Hotmoka can run on top of
Mokamint~\cite{mokamint_github}, that implements a proof of space consensus,
and of Tendermint~\cite{Tendermint}, that implements a proof of stake consensus.
Of course, you cannot mix such tools: if a blockchain network is running
on Mokamint, a new peer that joins the network
must use Mokamint, and the same for Tendermint.

Next section shows how you can earn some coins by mining for the
Mokamint-based main network of Hotmoka and how you can install a full node
of that main network, for earning even more coins. For now, we will
perform everything inside the docker container of Mokamint, so that
nothing must be installed in the local machine. Sec.~\ref{subsec:moka}
will show, later, how it is possible to install some utilities
in your local machine, so that you can run the commands directly there,
...
Fig.~\ref{fig:architecture} shows the architecture of a Hotmoka peer running on top of Mokamint.
...
Miners are not necessarily local, that is, providing themselves support for mining. They can also
be \emph{remote} miners, that can be imagined as plugs to which external miners can connect to help
...
in a temporary area called mempool, from where they get eventually selecxted for inclusion in a block.
Hotmoka is a separate component that runs transactions inside its database of states. This database
is where Java objects, including accounts, are stored. The architecture is similar if Tendermint is used
instead of Mokamint. The only difference is that Tendermint does not use miners, but creates new
blocks through validators.

The software components of Fig.~\ref{fig:architecture} are normally exposed through some IP ports, so that
remote users can interact with them. Interaction with Hotmoka is possible through the \texttt{moka} command-line utility.
Interaction with Mokamint's peer and miner is possible through a few utilities of Mokamint itself:
%
\begin{itemize}
\item \texttt{moka}\index{moka@{\texttt{moka}}} allows interaction with any Hotmoka node,
  independently from the underlying engine (Mokamint or Tendermint);
\item \texttt{mokamint-node}\index{mokamint-node@{\texttt{mokamint-node}}} allows one to interact with the Mokamint node;
\item \texttt{mokamint-miner}\index{mokamint-miners@{\texttt{mokamint-miners}}} allows one to interact with the miners of a Mokamint node;
...
The first command let us see which are the peers known by a node:
%
\index{mokamint-node@{\texttt{mokamint-node}}!{peers ls}@{\texttt{peers ls}}}
\inputCommand{docker_mokamintnode_peers_ls}
%
...
%
\inputOutput{docker_mokamintnode_peers_ls}
%
These are the peers connected to node \serverMokamintMainPublic{} of the main network.
You can, in turn, replace that URI with that of one of such peer, to see its peers.
...
\inputCommand{docker_mokamintnode_miners_ls}
%
The output tells us that there are two miners: a local and a remote one:
%
\inputOutput{docker_mokamintnode_miners_ls}
%
...
%
\section{The state of a Hotmoka node}\label{hotmoka_state}
...
might be allocated at \texttt{\accountOneShort{}}
...
\texttt{moka}\index{moka@{\texttt{moka}}} and a mobile client for Android, called Mokito\index{Mokito}. Mokito provides
basic functionalities only (handling accounts, querying the state of the objects in the node,
running simple transactions), while \texttt{moka} is the most complete solution.

\subsection{Moka}\label{subsec:moka}

You can use \texttt{moka} to interact with an existing Hotmoka node,
...
There are a few docker containers\index{docker!Hotmoka containers} providing \texttt{moka} and all the machinery
...
%
\inputCommand{docker_run_moka_objects_show_account1}
%
The output will be something like:
%
\inputOutput{docker_run_moka_objects_show_account1}
%
...
%
\index{moka@{\texttt{moka}}!version@{\texttt{version}}}
\inputCommand{moka_version}
%
and
%
\index{moka@{\texttt{moka}}!objects show@{\texttt{objects show}}}
\inputCommand{moka_objects_show_account1}
...
%
\index{moka@{\texttt{moka}}!help@{\texttt{help}}}
\inputOutput{moka_help}
%
...
%
\inputOutput{moka_objects_help_show}
...
it will connect to a default Hotmoka node
and show the screen in Fig.~\ref{fig:mokito_start}. The server can be changed
in the settings section of the app, accessible through the menu in the
top left area of the app.
%
\begin{figure}[t]
  \begin{center}
    \myincludegraphics{5cm}{mokito_start}
  \end{center}
...
%
\begin{commentbox}
The default Hotmoka node used by Mokito might be different in different versions of the app.
For the experiments that follow, we assume that you are connected to a node
of a test network, for instance to \serverMokamint{}. Check that is the case
in the settings page of the app, and change the node if needed.
\end{commentbox}
...
\serverMokamint{} and \serverTendermint{}, respectively.
...
at \serverMokamint. The result should be something like:
%
\inputOutput{moka_nodes_manifest_show}
%
...
in the case of \serverMokamint{} there are no validators, since that network
...
We have used \texttt{moka} to see the manifest of a node. You can also use the
...
\begin{subfigure}{.5\textwidth}
  \centering
  \myincludegraphics{.45\linewidth}{mokito_menu}
  \caption{The menu of the Mokito app.}
  \label{fig:mokito_menu}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \myincludegraphics{.45\linewidth}{mokito_manifest}
...
%
\index{moka@{\texttt{moka}}!keys create@{\texttt{keys create}}}
\inputCommand{moka_keys_create_account1}\index{key!creation}
\inputOutput{moka_keys_create_account1}
%
\begin{commentbox}
...
%
\index{moka@{\texttt{moka}}!accounts create@{\texttt{accounts create}}}
\inputCommand{moka_accounts_create_account1}\index{account!creation}
\inputOutput{moka_accounts_create_account1}
%
...
\index{moka@{\texttt{moka}}!objects show@{\texttt{objects show}}}
\inputCommand{moka_objects_show_account1}
\inputOutput{moka_objects_show_account1}
%
...
%
\index{moka@{\texttt{moka}}!accounts send@{\texttt{accounts send}}}
\inputCommand{moka_accounts_send_account1}
\inputOutput{moka_accounts_send_account1}
%
...
\begin{subfigure}{.5\textwidth}
  \centering
  \myincludegraphics{.45\linewidth}{mokito_new_account}
  \caption{The menu for creating a new account.}
  \label{fig:mokito_new_account}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \myincludegraphics{.45\linewidth}{mokito_elvis_new_account}
...
are just identified by their storage reference. For instance, \texttt{moka} currently does not
...
\begin{subfigure}{.5\textwidth}
  \centering
  \myincludegraphics{.45\linewidth}{mokito_show_elvis}
  \caption{The new account Elvis.}
  \label{fig:mokito_show_elvis}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \myincludegraphics{.45\linewidth}{mokito_added_elvis}
...
We have created \texttt{\accountOneShort{}} with \texttt{moka} and
...
%
\index{moka@{\texttt{moka}}!accounts import@{\texttt{accounts import}}}
\inputCommand{moka_accounts_import_account_mokito}
\inputOutput{moka_accounts_import_account_mokito}
%
After that, it is possible to control that account with \texttt{moka} (if we remember
...
\begin{subfigure}{.5\textwidth}
  \centering
  \myincludegraphics{.45\linewidth}{mokito_accounts_menu}
  \caption{The menu of the accounts screen.}
  \label{fig:mokito_accounts_menu}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \myincludegraphics{.45\linewidth}{mokito_insert_passphrase}
...
\begin{figure}[t]
  \begin{center}
    \myincludegraphics{5cm}{mokito_added_the_boss}
  \end{center}
...
%
\index{moka@{\texttt{moka}}!accounts export@{\texttt{accounts export}}}
\inputCommand{moka_accounts_export_account1}
\inputOutput{moka_accounts_export_account1}
%
...
%
\index{moka@{\texttt{moka}}!keys create@{\texttt{keys create}}}
\inputCommand{moka_keys_create_anonymous}
\inputOutput{moka_keys_create_anonymous}
%
\begin{commentbox}
...
%
\index{moka@{\texttt{moka}}!accounts send@{\texttt{accounts send}}}
\inputCommand{moka_accounts_send_account1_anonymous}\index{account!payment}\index{payment!to account}
\inputOutput{moka_accounts_send_account1_anonymous}
%
...
\inputCommand{moka_keys_bind_anonymous}
\inputOutput{moka_keys_bind_anonymous_not_yet}
%
Second attempt:
%
\inputCommand{moka_keys_bind_anonymous}
...
%
\inputCommand{moka_keys_bind_anonymous}
\inputOutput{moka_keys_bind_anonymous}
%
