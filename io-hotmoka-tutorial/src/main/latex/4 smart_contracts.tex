...
\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-ponzi\_simple} project in \hotmokaRepo{})
\end{center}
...
\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-ponzi\_annotations} project in \hotmokaRepo{})
\end{center}
...
\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-ponzi\_payable} project in \hotmokaRepo{})
\end{center}
...
\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-ponzi\_view} project in \hotmokaRepo{})
\end{center}
...
\begin{figure}[th!]
  \begin{center}
	...
	\scriptsize
	\classcolor
	\begin{abstractclass}[text width=2.8cm]{Storage}{-1,0}
		\operation{\# caller():Contract}
	\end{abstractclass}
	\ourclasscolor
	\begin{class}[text width=2.8cm]{@Exported Person}{3,-2}
		\inherit{Storage}
	\end{class}
	\classcolor
	\begin{abstractclass}[text width=3.8cm]{@Exported Contract}{-6,-2.1}
		\inherit{Storage}
		\attribute{- balance:BigInteger}
		\operation{@View balance():BigInteger}
	\end{abstractclass}
	\ourclasscolor
	\begin{class}[text width=2.8cm]{SimplePonzi}{-1.8,-3.5}
		\inherit{@Exported Contract}
	\end{class}
	\classcolor
	\begin{abstractclass}[text width=8.3cm]{PayableContract}{-5,-4.7}
		\inherit{@Exported Contract}
		\operation{@Payable @FromContract receive(amount:int|long|BigInteger)}
	\end{abstractclass}
	\interfacecolor
	\begin{interface}[text width=8cm]{Account}{4,-3}
		\operation{@View nonce():BigInteger}
		\operation{@View balance():BigInteger}
		\operation{@View publicKey():String}
		\operation{@FromContract(ExternallyOwnedAccount.class) rotatePublicKey(publicKey:String)}
	\end{interface}
	\begin{interface}[text width=2.8cm]{AccountED25519}{1,-6.5}
		\inherit{Account}
	\end{interface}
	\begin{interface}[text width=3.2cm]{AccountSHA256DSA}{6.5,-6.5}
		\inherit{Account}
	\end{interface}
	\begin{interface}[text width=2.8cm]{AccountQTESLA1}{2.2,-8}
		\inherit{Account}
	\end{interface}
	\begin{interface}[text width=2.8cm]{AccountQTESLA3}{5.7,-8}
		\inherit{Account}
	\end{interface}
	\classcolor
	\begin{class}[text width=8cm]{ExternallyOwnedAccount}{-5,-7}
		\inherit{PayableContract}
		\implement{Account}
		\attribute{- nonce:BigInteger}
		\attribute{- publicKey:String}
		\operation{ExternallyOwnedAccount()}
		\operation{@Payable @FromContract ExternallyOwnedAccount(initialAmount:int|long|BigInteger, publicKey:String)}
	\end{class}
	\begin{class}[text width=7cm]{ExternallyOwnedAccountED25519}{-5,-10.5}
		\inherit{ExternallyOwnedAccount}
		\implement{AccountED25519}
		\operation{ExternallyOwnedAccountED25519()}
		\operation{@Payable @FromContract ExternallyOwnedAccountED25519(initialAmount:int|long|BigInteger, publicKey:String)}
	\end{class}
	\begin{class}[text width=7.5cm]{ExternallyOwnedAccountSHA256DSA}{4,-10.5}
		\inherit{ExternallyOwnedAccount}
		\implement{AccountSHA256DSA}
		\operation{ExternallyOwnedAccountSHA256DSA()}
		\operation{@Payable @FromContract ExternallyOwnedAccountSHA256DSA(initialAmount:int|long|BigInteger, publicKey:String)}
	\end{class}
	\begin{class}[text width=6cm]{ExternallyOwnedAccountQTESLA1}{-5,-14.5}
		\inherit{ExternallyOwnedAccount}
		\implement{AccountQTESLA1}
		\operation{ExternallyOwnedAccountQTESLA1()}
		\operation{@Payable @FromContract ExternallyOwnedAccountQTESLA1(initialAmount:int|long|BigInteger, publicKey:String)}
	\end{class}
	\begin{class}[text width=6cm]{ExternallyOwnedAccountQTESLA3}{4,-14.5}
		\inherit{ExternallyOwnedAccount}
		\implement{AccountQTESLA3}
		\operation{ExternallyOwnedAccountQTESLA3()}
		\operation{@Payable @FromContract ExternallyOwnedAccountQTESLA3(initialAmount:int|long|BigInteger, publicKey:String)}
	\end{class}
	\end{tikzpicture}
  \end{center}
...
Such \emph{accounts} are typically controlled by humans, through a wallet
keeping its private key, but can be
...
we have used, as payer, an account created by the
\texttt{moka accounts create}\index{moka@{\texttt{moka}}!accounts create@{\texttt{accounts create}}} command,
...
used for the node. For that, it is enough to instantiate classes
...
(see~\ref{sec:signatures-and-quantum-resistance} later). This means that it is possible
