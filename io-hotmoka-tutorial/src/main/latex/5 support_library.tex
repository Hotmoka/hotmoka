...
\begin{figure}[th]
  \begin{center}
	...
	\end{interface}
	\externalclasscolor
	\begin{abstractclass}[text width=2cm]{Storage}{-0.5,-9}
	\end{abstractclass}
	\classcolor
...
\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-tictactoe} project in \hotmokaRepo{})
\end{center}
...
%
\index{moka@{\texttt{moka}}!objects create@{\texttt{objects create}}}
...
First investment:
%
\index{moka@{\texttt{moka}}!objects call@{\texttt{objects call}}}
...
\begin{figure}[th]
  \begin{center}
	...
	\end{interface}
	\externalclasscolor
	\begin{abstractclass}[text width=2cm]{Storage}{0,-9}
	\end{abstractclass}
	\classcolor
...
\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-tictactoe\_revised} project in \hotmokaRepo{})
\end{center}
...
One could hence define a new wrapper class for \texttt{byte}, that extends \texttt{Storage}.
...
(see next Sec.~\ref{subsec:blind_auction} for an example) and consequently deserve
...
\begin{figure}[th!]
  \begin{center}
	...
	\scriptsize
	\externalclasscolor
	\begin{interface}[text width=2.5cm]{Iterable\string<Byte\string>}{0,0}
	\end{interface}
	\interfacecolor
	\begin{interface}[text width=6cm]{StorageByteArrayView}{0,-1.5}
		\inherit{Iterable\string<Byte\string>}
		\operation{@View length():int}
		\operation{@View get(index:int):byte}
		\operation{toArray():byte[]}
		\operation{forEach(action:Consumer\string<? super Byte\string>)}
	\end{interface}
	\begin{interface}[text width=5.8cm]{SnapshottableStorageByteArrayView}{4.5,-6.5}
		\inherit{StorageByteArrayView}
		\operation{snapshot():StorageByteArrayView}
	\end{interface}
	\begin{interface}[text width=5.7cm]{StorageByteArray}{-4,-5.5}
		\inherit{StorageByteArrayView}
		\operation{set(index:int, value:byte)}
		\operation{update(index:int, how:IntUnaryOperator)}
		\operation{view():StorageByteArrayView}
	\end{interface}
	\begin{interface}[text width=5.7cm]{SnapshottableStorageByteArray}{4.5,-8.7}
		\inherit{StorageByteArray}
		\inherit{SnapshottableStorageByteArrayView}
		\operation{view():SnapshottableStorageByteArrayView}
	\end{interface}
	\externalclasscolor
	\begin{abstractclass}[text width=2cm]{Storage}{0,-8.7}
	\end{abstractclass}
	\classcolor
	\begin{class}[text width=4.4cm]{StorageTreeByteArray}{-6,-9.5}
		\inherit{Storage}
		\implement{StorageByteArray}
		\operation{StorageTreeByteArray(length:int)}
		\operation{StorageTreeByteArray(length:int, initialValue:byte)}
	\end{class}
	\begin{class}[text width=8.7cm]{SnapshottableStorageTreeByteArray}{-4.5,-12.5}
		\inherit{Storage}
		\implement{SnapshottableStorageByteArray}
		\operation{SnapshottableStorageTreeByteArray(length:int)}
		\operation{SnapshottableStorageTreeByteArray(length:int, initialValue:byte)}
	\end{class}
	\begin{class}[text width=4.5cm]{Bytes32}{5.7,-12.5}
		\inherit{Storage}
		\implement{SnapshottableStorageByteArray}
		\operation{Bytes32()}
		\operation{Bytes32(byte0:byte,...,byte31:byte)}
		\operation{snapshot():Bytes32Snapshot}
	\end{class}
	\begin{class}[text width=5.7cm]{@Exported Bytes32Snapshot}{2.5,-14.5}
		\inherit{Storage}
		\implement{StorageByteArrayView}
		\operation{Bytes32Snapshot(byte0:byte,...,byte31:byte)}
	\end{class}
	\end{tikzpicture}
  \end{center}
  \caption{The hierarchy of storage byte arrays.}
  \label{fig:byte_arrays_hierarchy}
\end{figure}

Fig.~\ref{fig:byte_arrays_hierarchy} shows the hierarchy of the specialized classes for arrays of bytes,
...
\begin{figure}[th]
  \begin{center}
	...
	\end{interface}
	\externalclasscolor
	\begin{abstractclass}[text width=2cm]{Storage}{-1,-11}
	\end{abstractclass}
	\classcolor
...
\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-auction} project in \hotmokaRepo{})
\end{center}
...
\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-auction\_events} project in \hotmokaRepo{})
\end{center}
...
contract of the previous section. We could run it in the \serverMokamint{} server,
...
for the bidding and for the revealing phases. Instead, we use the \serverTendermint{} server,
...
\serverMokamint{} do not exist in \serverTendermint{}. We do it as previously done,
