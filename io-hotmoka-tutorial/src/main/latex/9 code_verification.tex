...
\item Classes that extend \texttt{io.takamaka.code.lang.Storage}\index{Storage@{\texttt{Storage}}}
    ...
    (see Sec.~\ref{sec:storage_types_and_classes}).
%
\begin{commentbox}
...
actually contains a storage value or not (see the dynamic check number~\ref{check:object_or_interface_fields} at the end of this section).
...
\item Non-transient fields of type \texttt{java.lang.Object} or of type interface,
   ...
   at the end of the transaction, contain \texttt{null} or a storage
   object~\label{check:object_or_interface_fields}.
\end{enumerate}

\section{Command-line verification and instrumentation}

\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-family\_errors} project in \hotmokaRepo{})
\end{center}
...
that we should find in the cache of Maven:\index{moka@{\texttt{moka}}!jars verify@{\texttt{jars verify}}}
%
\inputCommand{moka_jars_verify_takamaka}
\inputOutput{moka_jars_verify_takamaka}
%
...
installation in a Hotmoka node. For that, we run:\index{moka@{\texttt{moka}}!jars instrument@{\texttt{jars instrument}}}
%
\inputCommand{moka_jars_instrument_takamaka}
\inputOutput{moka_jars_instrument_takamaka}
%
...
(that is, the correct version of the project).
As all Takamaka programs, that jar uses classes from the \texttt{io-takamaka-code} jar,
hence it depends on it. We specify this with the \texttt{-{}-libs} option, that must
refer to an already instrumented jar:\index{moka@{\texttt{moka}}!jars instrument@{\texttt{jars instrument}}}
%
\inputCommand{moka_jars_instrument_family}
\inputOutput{moka_jars_instrument_family}
%
...
be printed on the screen:\index{moka@{\texttt{moka}}!jars verify@{\texttt{jars verify}}}
%
\inputCommand{moka_jars_verify_family_errors}
\inputOutput{moka_jars_verify_family_errors}
%
The same failure occurs with the \texttt{moka jars instrument} command,
that will not generate the instrumented jar. It only
reports the first encountered error before failure:\index{moka@{\texttt{moka}}!jars instrument@{\texttt{jars instrument}}}
%
\inputCommand{moka_jars_instrument_family_errors}
\inputOutput{moka_jars_instrument_family_errors}