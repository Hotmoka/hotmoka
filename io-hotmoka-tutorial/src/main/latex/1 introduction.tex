...
Almost two decades ago, Bitcoin~\cite{Nakamoto08}\index{Bitcoin}
...
\emph{consensus} mechanism based on a
\emph{proof of work}\index{proof of work}, while the
...
That was Ethereum~\cite{AntonopoulosWPMP25}\index{Ethereum}, whose
...
in the last years~\cite{BeniniGMS21,CrosaraOST21,CrosaraOST23,OlivieriST21,Spoto19,Spoto20,SpotoMGB23}.
...
free to write to us at \faustoEmail{}, with bugs and improvement requests.
...
\section{The example projects of this book}\label{sec:example_projects}
...
%
\inputCommand{git_clone_hotmoka}
%
The examples will be inside the Maven module
\texttt{io-hotmoka-tutorial-examples}.

\section{Installing the source code of Hotmoka}\label{sec:installing_source_code}

You do \emph{not} need to install to source code of Hotmoka to follow this
book. Nevertheless, an important aspect of blockchain technology is that
trust is based also on the public availability of the source code.
Moreover, you will need to download the source code if you want to understand
the inner details of Hotmoka, or contribute to the development of the project or
fork the project.

Assuming that you have already downloaded the source code of Hotmoka, as shown
in Sec.~\ref{sec:example_projects}, you can \texttt{cd} to the \texttt{hotmoka}
directory and compile, package, test and install the Hotmoka jars:
%
\inputCommand{mvn_clean_install}
%
All tests should pass and all projects should be successfully compiled and packaged.

\begin{commentbox}
  If you are not interested in running the tests, append \texttt{-DskipTests} at the end
  of the previous command.
\end{commentbox}

If you want to edit the source code inside an IDE, you can import it in Eclipse\index{Eclipse},
NetBeans\index{NetBeans} or IntelliJ\index{IntelliJ}.
In Eclipse, use the \emph{File$\rightarrow$Import$\rightarrow$Existing Maven Projects}
menu item and import
the parent Maven project contained in the \texttt{hotmoka} directory that you cloned from
...
\emph{Run As} and then the \emph{Maven install} target. You will also be able
to run the tests inside
the Eclipse JUnit runner, by right-clicking on the \texttt{io-hotmoka-tests} subproject
and selecting \emph{Run As} and then the \emph{JUnit Test} target.

\section{Available Hotmoka nodes}\label{sec:available_hotmoka_nodes}

Most experiments in this book will be performed against
Hotmoka nodes of already existing test networks, maintained
by Hotmoka itself.
They can be contacted at their respective endpoints and
publish a \emph{faucet}\index{faucet}, that is, a free source of
cryptocurrency. They are reported in Fig.~\ref{fig:endpoints}.
The same figure reports
the endpoints of a mainnet node, that can be contacted as well, but requires
to hold some cryptocurrency in order to run updating transactions, since
it does not publish any faucet.

\begin{figure}[th]
\begin{center}
\begin{tabular}{|c||c|c|l|}
\hline
network                                & chain id                                           & interface & endpoint \\\hline\hline
\multirow{3}{*}{Mokamint test network} & \multirow{3}{*}{\texttt{\serverMokamintChainId{}}} & Hotmoka node & \texttt{\serverMokamint{}} \\\cline{3-4}
\mrowcell                              & \mrowcell                                          & Mokamint node & \texttt{\serverMokamintPublic{}} \\\cline{3-4}
\mrowcell                              & \mrowcell                                          & Mokamint mining & \texttt{\serverMokamintMining{}} \\\hline\hline
\multirow{3}{*}{Mokamint main network} & \multirow{3}{*}{\texttt{\serverMokamintMainChainId{}}} & Hotmoka node & \texttt{\serverMokamintMain{}} \\\cline{3-4}
\mrowcell                              & \mrowcell                                              & Mokamint node & \texttt{\serverMokamintMainPublic{}} \\\cline{3-4}
\mrowcell                              & \mrowcell                                              & Mokamint mining & \texttt{\serverMokamintMainMining{}} \\\hline\hline
\multirow{2}{*}{Tendermint test network} & \multirow{2}{*}{\texttt{\serverTendermintChainId{}}} & Hotmoka node & \texttt{\serverTendermint{}} \\\cline{3-4}
\mrowcell                                & \mrowcell                                            & Tendermint whisper & \texttt{\serverTendermintWhisper{}} \\\hline
\end{tabular}
\end{center}
\caption{The endpoints of the existing Hotmoka nodes that can be used for our experiments.}\label{fig:endpoints}
\end{figure}
	
\section{About the author}\label{sec:about_the_author}
...
Ch.~\ref{ch:tokens} is shared work with Marco Crosara,
Filippo Fantinato, Luca Olivieri and Fabio Tagliaferro.
Ch.~\ref{ch:hotmoka_nodes} has been
inspired by previous work with Dinu Berinde.
Sec.~\ref{subsec:shared_entities} is shared work
with Andrea Benini, Mauro Gambini and Sara Migliorini.

\begin{center}
  \myincludegraphics{3cm}{dockerhub}
\end{center}

Hotmoka enjoys being a Docker-sponsored\index{docker} open source
...
\begin{center}
  \myincludegraphics{3cm}{github}
\end{center}

GitHub\index{GitHub} (\url{https://github.com})
...
\begin{center}
  \myincludegraphics{3cm}{YourKit}
\end{center}

Hotmoka benefits from the use of a free license of
the YourKit\index{YourKit} profiler
