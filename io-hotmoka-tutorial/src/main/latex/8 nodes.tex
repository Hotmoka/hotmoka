...
form from \website{https://github.com/tendermint/tendermint/releases/tag/v\tendermintVersion}.
...
%
\index{moka@{\texttt{moka}}!nodes tendermint init@{\texttt{nodes tendermint init}}}
...
%
\index{moka@{\texttt{moka}}!keys bind@{\texttt{keys bind}}}\index{key!binding}
...
%
\index{moka@{\texttt{moka}}!nodes faucet@{\texttt{nodes faucet}}}
...
\serverMokamint{} with \url{ws://my.machine:8001}: your computer will serve
...
%
\index{moka@{\texttt{moka}}!nodes tendermint resume@{\texttt{nodes tendermint resume}}}
...
\begin{figure}[th!]
  \begin{center}
	...
	\scriptsize
	\classcolor
	\begin{abstractclass}[text width=2.2cm]{Storage}{4,0}
	\end{abstractclass}
	\begin{abstractclass}[text width=2.2cm]{Contract}{7,-1}
		\inherit{Storage}
		\operation{@View balance():BigInteger}
	\end{abstractclass}
	\begin{abstractclass}[text width=4.2cm]{PayableContract}{6.7,-3}
		\inherit{Contract}
		\operation{@Payable @FromContract receive(amount:BigInteger)}		
	\end{abstractclass}
	\begin{class}[text width=4.2cm]{ExternallyOwnedAccount}{6.5,-5.5}
		\inherit{PayableContract}
		\operation{@View nonce():BigInteger}		
		\operation{@View publicKey():String}		
	\end{class}
	\begin{class}[text width=3.7cm]{Validator}{7,-7.5}
		\inherit{ExternallyOwnedAccount}
		\operation{Validator(publicKey:String)}
		\operation{@View id():String}
	\end{class}
	\begin{class}[text width=6.5cm]{TendermintED25519Validator}{5.5,-9.8}
		\inherit{Validator}
		\operation{TendermintED25519Validator(publicKey:String)}
	\end{class}
	\begin{class}[text width=3.7cm]{Offer\string<S extends PayableContract\string>}{2.2,-1.5}
		\inherit{Storage}
		\attribute{seller:S}
		\attribute{sharesOnSale:BigInteger}
		\attribute{cost:BigInteger}
		\operation{@View isOngoing():boolean}
	\end{class}	
	\interfacecolor
	\begin{interface}[text width=6.9cm]{SEV}{-3.8,0} % a trick to allow extension of classes with complex generics
		\operation{@View getShares():StorageMapView\string<S,BigInteger\string>}
		\operation{@View isShareholder(who:Object):boolean}
		\operation{@View sharesOf(shareholder:S):BigInteger}
		\operation{snapshot():SharedEntityView\string<S\string>}
	\end{interface}
	\begin{interface}[text width=6.9cm]{SharedEntityView\string<S extends Contract\string>}{-3.8,0}
		\operation{@View getShares():StorageMapView\string<S,BigInteger\string>}
		\operation{@View isShareholder(who:Object):boolean}
		\operation{@View sharesOf(shareholder:S):BigInteger}
		\operation{snapshot():SharedEntityView\string<S\string>}
	\end{interface}
	\begin{interface}[text width=9.4cm]{SE}{-3,-5.3}
		\operation{@View getOffers():StorageSetView\string<O\string>}
		\operation{@View sharesOnSaleOf(shareholder:S):BigInteger}
		\operation{@FromContract @Payable place(amount:BigInteger, offer:O)}
		\operation{@FromContract @Payable accept(amount:BigInteger, buyer:S, offer:O)}
		\operation{view():SharedEntityView\string<S\string>}
	\end{interface}
	\begin{interface}[text width=9.4cm]{SharedEntity\string<S extends PayableContract O extends Offer\string<S\string>{}\string>}{-3,-5}
		\inherit{SEV}
		\operation{@View getOffers():StorageSetView\string<O\string>}
		\operation{@View sharesOnSaleOf(shareholder:S):BigInteger}
		\operation{@FromContract @Payable place(amount:BigInteger, offer:O)}
		...
		\operation{view():SharedEntityView\string<S\string>}
	\end{interface}
	\begin{interface}[text width=6.5cm]{Validators\string<V extends Validator\string>}{-3,-9}
		\inherit{SE}
		\operation{@View getStake(validator:V):BigInteger}
		\operation{@View getInitialSupply():BigInteger}
		\operation{@View getCurrentSupply():BigInteger}
		\operation{@View getFinalSupply():BigInteger}
		\operation{@View getHeight():BigInteger}
		\operation{@View getNumberOfTransactions():BigInteger}
	\end{interface}
	\classcolor
	\begin{class}[text width=7cm]{SSE}{2,-13.3}
		\operation{SimpleSharedEntity(shareholder:S, share:BigInteger)}
	\end{class}
	\begin{class}[text width=7cm]{SimpleSharedEntity\string<S extends PayableContract O extends Offer\string<S\string>{}\string>}{2,-13}
		\inherit{PayableContract}
		\implement{SE}
		\operation{SimpleSharedEntity(shareholder:S, share:BigInteger)}
	\end{class}
	\begin{abstractclass}[text width=6.7cm]{AbstractValidators\string<V extends Validator\string>}{-2,-15.5}
		\inherit{SSE}
		\implement{Validators\string<V extends Validator\string>}
	\end{abstractclass}
	\begin{class}[text width=17.6cm]{TendermintValidators}{0,-17.5}
		\inherit{AbstractValidators\string<V extends Validator\string>}
		\operation{TendermintValidators(validator:TendermintED25519Validator, power:BigInteger)}
		\operation{@FromContract @Payable accept(amount:BigInteger,buyer:TendermintED25519Validator,offer:Offer\string<TendermintED25519Validator\string>)}
	\end{class}
	\end{tikzpicture}
  \end{center}
...
among \emph{shareholders} (see also~\cite{SpotoMGB23}). Fig.~\ref{fig:entities_hierarchy} shows this
...
is just a special case of shared entity.
...
the \texttt{snapshot()}\index{snapshot()@{\texttt{snapshot()}}}
...
in the blocks and easy to eavesdrop.
...
at stake for each given validator (if the validator misbehaves, its stake will be slashed).
...
%
\index{moka@{\texttt{moka}}!nodes disk init@{\texttt{nodes disk init}}}
...
\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-runs} project in \hotmokaRepo{})
\end{center}
...
%
\index{moka@{\texttt{moka}}!jars install@{\texttt{jars install}}}
