...
\begin{center}
(See the \texttt{io-hotmoka-tutorial-examples-family} project in \hotmokaRepo{})
\end{center}
...
\begin{commentbox}
We are using \takamakaVersion{} here, as version of the Takamaka runtime
...
\begin{figure}[t]
  \begin{center}
    \myincludegraphics{5cm}{family_jar}
  \end{center}
...
%
\inputCommand{moka_jars_install}\index{moka@{\texttt{moka}}!jars install@{\texttt{jars install}}}\index{jar!install}
\inputOutput{moka_jars_install}
%
...
%
\inputCommand{mvn_exec_family_1}
\inputOutput{mvn_exec_family_1}
%
...
\begin{center}
  ...
  \texttt{io-hotmoka-tutorial-examples-runs} projects in \hotmokaRepo{})
\end{center}
...
new \texttt{Person} object into the store of the node. We can perform this through the moka tool:\index{moka@{\texttt{moka}}!objects create@{\texttt{objects create}}}
%
\inputCommand{moka_objects_create_person_failed}
\inputOutput{moka_objects_create_person_failed}
%
...
Install this new version in blockchain:\index{moka@{\texttt{moka}}!jars install@{\texttt{jars install}}}
%
\inputCommand{moka_jars_install_2}
\inputOutput{moka_jars_install_2}
%
then run the \texttt{moka objects create} command again. This time, the execution should
complete without exception:\index{moka@{\texttt{moka}}!objects create@{\texttt{objects create}}}
%
\inputCommand{moka_objects_create_person}
\inputOutput{moka_objects_create_person}
%
...
by using the \texttt{moka objects show} command:\index{moka@{\texttt{moka}}!objects show@{\texttt{objects show}}}
%
\inputCommand{moka_objects_show_person}
\inputOutput{moka_objects_show_person}
%
\begin{commentbox}
...
%
\inputCommand{mvn_exec_family_storage}
\inputOutput{mvn_exec_family_storage}
%
...
\inputCommand{moka_objects_call_toString}
\inputOutput{moka_objects_call_toString}
%
...
\inputCommand{moka_jars_install_3}
\inputOutput{moka_jars_install_3}
%
then create a new \texttt{Person} object:
%
\index{moka@{\texttt{moka}}!objects create@{\texttt{objects create}}}
\inputCommand{moka_objects_create_person_exported}
\inputOutput{moka_objects_create_person_exported}
%
and finally call \texttt{toString()} on that new object:
%
\index{moka@{\texttt{moka}}!objects call@{\texttt{objects call}}}
\inputCommand{moka_objects_call_toString_exported}
\inputOutput{moka_objects_call_toString_exported}
%
...
%
\inputCommand{mvn_exec_family_exported}
\inputOutput{mvn_exec_family_exported}
...
\section{Storage types and constraints on storage classes}\label{sec:storage_types_and_classes}
