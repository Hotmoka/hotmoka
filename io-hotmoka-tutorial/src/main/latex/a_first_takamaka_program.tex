...
Since class \texttt{Person} has been modified, you need to regenerate the jar file
\texttt{io-hotmoka-tutorial-examples-family-\hotmokaVersion{}.jar},
...
must be annotated as \texttt{@Exported}\index{Exported}.
...
This includes all contracts since the class \texttt{io.takamaka.code.lang.Contract}, that
...
\texttt{MethodSignatures.ofNonVoid(StorageTypes.OBJECT, "toString", StorageTypes.STRING)}\linebreak
instead of
...
\item The values that can be exchanged from outside the node to
  ...
  that objects must belong to an \texttt{@Exported} class.\index{Exported}
\end{enumerate}
...
annotated as \texttt{@View}\index{View} can be performed through the
