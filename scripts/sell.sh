...
echo "The seller of the crypto is the account $SELLER_ADDRESS."
echo "The seller of the validation power is the validator $VALIDATOR_ADDRESS."
...
cd $DIR
RUN=$(./${CLI}/${CLI} info --url $NETWORK_URL)
...
read -p "     ask the buyer which key to pay into and enter that key here: " MONEY_ACCOUNT_PUBLIC_KEY_BASE58
read -s -p "     enter the password of the seller account: " PASSWORD_OF_SELLER
echo
cd $DIR
./${CLI}/${CLI} send $FUND_AMOUNT $MONEY_ACCOUNT_PUBLIC_KEY_BASE58 --anonymous --payer $SELLER_ADDRESS --url $NETWORK_URL --interactive=false --password-of-payer=$PASSWORD_OF_SELLER
cd ..
