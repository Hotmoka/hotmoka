...
public interface InitializedNode extends Node {
	...
	/**
	 * Yields a decorated node with basic Takamaka classes, gamete and manifest.
	 * Uses the given keys to control the gamete. It allows to specify the validators
	 * of the node's network, that will get stored in the manifest object created
	 ...
	 * @throws NoSuchAlgorithmException if the signing algorithm for the node is not available in the Java installation
	 */
	static InitializedNode of(Node parent, KeyPair keysOfGamete, Stream<Validator> validators, Path takamakaCode, String manifestClassName, String chainId, BigInteger greenAmount, BigInteger redAmount) throws TransactionRejectedException, TransactionException, CodeExecutionException, IOException, InvalidKeyException, SignatureException, NoSuchAlgorithmException {
		return new InitializedNodeImpl(parent, keysOfGamete, validators, takamakaCode, manifestClassName, chainId, greenAmount, redAmount);
	}
}