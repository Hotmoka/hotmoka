...
not white-listed, since their behaviour is client-dependent.
...
Takamaka,
rephrased from a Solidity example~\cite{solidity}.
...
\<StorageList> class, that extends \<Storage> and can then be persisted to blockchain.
...
it becomes its payer and can send money along.
Takamaka checks that \<@Entry> methods belong
to classes that extend \<takamaka.lang.Contract> and that they
are only called from a distinct contract object.
Inside \<@Entry> methods or constructors, it is possible to call method
\<payer>, that returns the calling contract.
...
It means that the contract receives money from the \<payer> contract.
...
that money from \<payer> to the destination contract, at call time.
...
\<null> in RAM.
As the transaction proceeds, in RAM and inside the Java Virtual Machine, and
...
that the their instance fields must be primitive or have storage class, recursively.
...
\<onStorage> field holds true and its
...
existed. To avoid this, the actual instrumentation will use
...
public abstract class Contract extends Storage {
  ...
  private transient Contract payer; // not kept in blockchain  §\label{line:payer_field}§
  ...
  protected final void entry(Contract payer) { §\label{line:entry_method}§
    require(this != payer, "@Entry must be called by a distinct object");
    this.payer = payer;
  }
...
Methods \<require> can be used to check for specific conditions from inside a contract.
...
caller (\<payer>) are distinct contract objects, then records the
\<payer> of the callee. Method \<payableEntry> does the same and,
...
\end{lstlisting}
%
Similarly, for every \<@Payable @Entry> method:
%
\begin{lstlisting}[numbers=none,frame=none]
  public @Payable @Entry T m(int amount, args) { body }
\end{lstlisting}
%
Takamaka verifies that it actually has a first formal parameter of type \<int>
(the amount of transferred money) and instruments it into
%
\begin{lstlisting}[numbers=none,frame=none]
...
a public constructor or method of another contract.
The caller must specify an amount of gas for the transaction.
Takamaka will run the code of the constructor or method, withdrawing money
...
the transaction, an unchecked \<takamaka.lang.OutOfGasException> is thrown.
...
\<tick> throws an \<OutOfGasException>. The chosen \<amount> depends
on the instruction being instrumented, so that instructions of different
execution cost can have different gas cost.

Exceptions \<OutOfGasException>s cannot be caught:
Takamaka extends every exception table in the code with an
extra, initial handler for \<OutOfGasException>, that
simply rethrows the exception.
This prevents possible DOS attacks, that might catch the \<OutOfGasException>
and lead to the execution of an infinite loop when the gas expires.
