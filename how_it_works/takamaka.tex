...
However, Takamaka will check at run time that such objects hava actually storage class
...
will not be serialized later and will be lost.
...
much more restrictive than the same sandbox. Moreover, Java provides object serialization
...
executed; termination, since gas is metered and the \<OutOfGasException> cannot
be caught; and isolation, since the Java Virtual Machine enforces that Java's visibility
...
Takamaka uses the JVM, that is more optimised than the Ethereum virtual machine, but is also
more heavy-weight at start-up. It will not be possible to start a JVM for each transaction.
