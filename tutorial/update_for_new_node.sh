...
sed -i "/@docker_takamaka_code/s/\/.*\//\/@docker_takamaka_code\/$DOCKER_TAKAMAKA_CODE\//" create_from_source.sh

LINE=$(echo "$RUN" | grep manifest)
DOCKER_MANIFEST=${LINE: -66}
echo "  docker's manifest = $DOCKER_MANIFEST"
sed -i "/@docker_manifest/s/\/.*\//\/@docker_manifest\/$DOCKER_MANIFEST\//" create_from_source.sh

LINE=$(echo "$RUN" | grep gamete)
DOCKER_GAMETE=${LINE: -66}
echo "  docker's gamete = $DOCKER_GAMETE"
sed -i "/@docker_gamete/s/\/.*\//\/@docker_gamete\/$DOCKER_GAMETE\//" create_from_source.sh

LINE=$(echo "$RUN" | grep "validators" | sed '1!d')
DOCKER_VALIDATORS=${LINE: -66}
echo "  docker's validators = $DOCKER_VALIDATORS"
sed -i "/@docker_validators/s/\/.*\//\/@docker_validators\/$DOCKER_VALIDATORS\//" create_from_source.sh

LINE=$(echo "$RUN" | grep "validator #0" | sed '1!d')
DOCKER_VALIDATOR0=${LINE: -66}
echo "  docker's validator #0 = $DOCKER_VALIDATOR0"
sed -i "/@docker_validator0/s/\/.*\//\/@docker_validator0\/$DOCKER_VALIDATOR0\//" create_from_source.sh

LINE=$(echo "$RUN" | grep "id:" | sed '1!d')
DOCKER_ID_VALIDATOR0=${LINE: -40}
echo "  docker's validator #0 id = $DOCKER_ID_VALIDATOR0"
sed -i "/@docker_id_validator0/s/\/.*\//\/@docker_id_validator0\/$DOCKER_ID_VALIDATOR0\//" create_from_source.sh
...
sed -i "/@docker_new_account/s/\/.*\//\/@docker_new_account\/$DOCKER_NEW_ACCOUNT\//" create_from_source.sh
...
sed -i "/@docker_total_gas_new_account/s/\/.*\//\/@docker_total_gas_new_account\/$DOCKER_TOTAL_GAS_NEW_ACCOUNT\//" create_from_source.sh
...
sed -i "/@docker_reduced_balance/s/\/.*\//\/@docker_reduced_balance\/$DOCKER_REDUCED_BALANCE\//" create_from_source.sh
...
sed -i "/@docker_balance_validator0/s/\/.*\//\/@docker_balance_validator0\/$DOCKER_BALANCE_VALIDATOR0\//" create_from_source.sh
...
sed -i "/@docker_staked_validator0/s/\/.*\//\/@docker_staked_validator0\/$DOCKER_STAKED_VALIDATOR0\//" create_from_source.sh
let DOCKER_DIFF1=$DOCKER_TOTAL_SUPPLY-$DOCKER_REDUCED_BALANCE
sed -i "/@docker_diff1/s/\/.*\//\/@docker_diff1\/$DOCKER_DIFF1\//" create_from_source.sh
let DOCKER_DIFF2=$DOCKER_DIFF1-1234567
sed -i "/@docker_diff2/s/\/.*\//\/@docker_diff2\/$DOCKER_DIFF2\//" create_from_source.sh
let DOCKER_SUM1=$DOCKER_BALANCE_VALIDATOR0+$DOCKER_STAKED_VALIDATOR0
sed -i "/@docker_sum1/s/\/.*\//\/@docker_sum1\/$DOCKER_SUM1\//" create_from_source.sh
let DOCKER_DIFF3=$DOCKER_SUM1-$DOCKER_DIFF2
sed -i "/@docker_diff3/s/\/.*\//\/@docker_diff3\/$DOCKER_DIFF3\//" create_from_source.sh
