...
public class ResolverImpl implements Resolver {
	...
	public Optional<Field> resolvedFieldFor(FieldInstruction fi) {
		...
		if (holder instanceof ObjectType) {
			...
			Class<?> type = clazz.jar.getBcelToClass().of(fi.getFieldType(cpg));
	
			return ThrowIncompleteClasspathError.insteadOfClassNotFoundException
				(() -> clazz.jar.getClassLoader().resolveField(((ObjectType) holder).getClassName(), name, type));
		}
	...
	public Optional<? extends Executable> resolvedExecutableFor(InvokeInstruction invoke) {
		...
		Class<?>[] args = clazz.jar.getBcelToClass().of(invoke.getArgumentTypes(cpg));
		...
		if (invoke instanceof INVOKESPECIAL && Const.CONSTRUCTOR_NAME.equals(methodName))
			...
			Class<?> returnType = clazz.jar.getBcelToClass().of(invoke.getReturnType(cpg));
	...
	Optional<Constructor<?>> resolveConstructorWithPossiblyExpandedArgs(String className, Class<?>[] args) {
		return ThrowIncompleteClasspathError.insteadOfClassNotFoundException(() -> {
			Optional<Constructor<?>> result = clazz.jar.getClassLoader().resolveConstructor(className, args);
			...
			return result.isPresent() ? result : clazz.jar.getClassLoader().resolveConstructor(className, expandArgsForEntry(args));
		});
	}
	...
	Optional<java.lang.reflect.Method> resolveMethodWithPossiblyExpandedArgs(String className, String methodName, Class<?>[] args, Class<?> returnType) {
		return ThrowIncompleteClasspathError.insteadOfClassNotFoundException(() -> {
			Optional<java.lang.reflect.Method> result = clazz.jar.getClassLoader().resolveMethod(className, methodName, args, returnType);
			return result.isPresent() ? result : clazz.jar.getClassLoader().resolveMethod(className, methodName, expandArgsForEntry(args), returnType);
		});
	}
	...
	Optional<java.lang.reflect.Method> resolveInterfaceMethodWithPossiblyExpandedArgs(String className, String methodName, Class<?>[] args, Class<?> returnType) {
		return ThrowIncompleteClasspathError.insteadOfClassNotFoundException(() -> {
			Optional<java.lang.reflect.Method> result = clazz.jar.getClassLoader().resolveInterfaceMethod(className, methodName, args, returnType);
			return result.isPresent() ? result : clazz.jar.getClassLoader().resolveInterfaceMethod(className, methodName, expandArgsForEntry(args), returnType);
		});
	}
