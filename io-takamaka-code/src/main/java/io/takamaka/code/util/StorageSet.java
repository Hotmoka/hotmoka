...
public class StorageSet<V> extends Storage implements Iterable<V> {
	...
	private static <V> Node<V> floorKey(Node<V> x, V value) {
	...
	private static <V> Node<V> ceilingKey(Node<V> x, V value) {  
		if (x == null) return null;
		int cmp = compareTo(value, x.value);
		if (cmp == 0) return x;
	...
	private static <V> int rank(V value, Node<V> x) {
		if (x == null) return 0; 
		int cmp = compareTo(value, x.value); 
	...
	public Iterator<V> iterator() {
		return new StorageSetIterator<V>(root);
	}
