	...
	public InstrumentMethodsOfSupportClasses(InstrumentedClassImpl.Builder builder, MethodGen method) {
		...
		if (className.equals(Constants.STORAGE_NAME)) {
			...
			if ("mint".equals(method.getName())) {
				...
				il.append(InstructionConst.ALOAD_0);
				il.append(InstructionConst.ALOAD_1);
				il.append(factory.createInvoke(Constants.RUNTIME_NAME, "mint", Type.VOID, new Type[] { Type.OBJECT, Type.OBJECT, BIGINTEGER_OT }, Const.INVOKESTATIC));
				il.append(InstructionConst.RETURN);
				method.setInstructionList(il);
			}
				...
				il.append(InstructionConst.ALOAD_0);
				il.append(InstructionConst.ALOAD_1);
				il.append(factory.createInvoke(Constants.RUNTIME_NAME, "burn", Type.VOID, new Type[] { Type.OBJECT, Type.OBJECT, BIGINTEGER_OT }, Const.INVOKESTATIC));
				il.append(InstructionConst.RETURN);
				method.setInstructionList(il);
			}
		}
