...
class Signatures extends TakamakaTest {
	...
	void createAccountsWithDistinctSigningAlgorithmsAndUseThem() throws TransactionException, CodeExecutionException, TransactionRejectedException, InvalidKeyException, SignatureException, NoSuchAlgorithmException {
		...
		BigIntegerValue qteslaResult = (BigIntegerValue) node.addStaticMethodCallTransaction(new StaticMethodCallTransactionRequest(Signer.with(qtesla1, qteslaKeyPair), qteslaAccount, ZERO, chainId, _100_000, ONE, takamakaCode(), callee, new LongValue(1973)));
		...
		BigIntegerValue ed25519Result = (BigIntegerValue) node.addStaticMethodCallTransaction(new StaticMethodCallTransactionRequest(Signer.with(ed25519, ed25519KeyPair), ed25519Account, ZERO, chainId, _100_000, ONE, takamakaCode(), callee, new LongValue(1973)));
