...
class FromContracts extends HotmokaTest {
	...
	void callFromContract2() throws Exception {
		StorageReference entries = addConstructorCallTransaction(privateKey(0), account(0), _500_000, ONE, jar(), ConstructorSignatures.of(FROM_CONTRACTS));
		StorageValue result = addInstanceNonVoidMethodCallTransaction(privateKey(0), account(0), _500_000, ONE, jar(), MethodSignatures.ofNonVoid(FROM_CONTRACTS, "entry2", StorageTypes.CONTRACT), entries);
		assertEquals(entries, result);
	}

	@Test @DisplayName("new FromContracts().entry3() yields the FromContracts object itself")
	void callFromContract3() throws Exception {
		StorageReference entries = addConstructorCallTransaction(privateKey(0), account(0), _500_000, ONE, jar(), ConstructorSignatures.of(FROM_CONTRACTS));
		StorageValue result = addInstanceNonVoidMethodCallTransaction(privateKey(0), account(0), _500_000, ONE, jar(), MethodSignatures.ofNonVoid(FROM_CONTRACTS, "entry3", StorageTypes.CONTRACT), entries);
		assertEquals(entries, result);
	}

	@Test @DisplayName("new FromContracts().entry4() yields the FromContracts object itself")
	void callFromContract4() throws Exception {
		StorageReference entries = addConstructorCallTransaction(privateKey(0), account(0), _500_000, ONE, jar(), ConstructorSignatures.of(FROM_CONTRACTS));
		StorageValue result = addInstanceNonVoidMethodCallTransaction(privateKey(0), account(0), _500_000, ONE, jar(), MethodSignatures.ofNonVoid(FROM_CONTRACTS, "entry4", StorageTypes.CONTRACT), entries);
		assertEquals(entries, result);
	}
